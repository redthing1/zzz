...
use walkdir::WalkDir;
use anyhow::Context;
use crate::formats::{CompressionFormat, CompressionOptions, ExtractionOptions, CompressionStats, ArchiveEntry};
...
impl ZstdFormat {
    ...
    fn collect_files_to_add(
        ...
        if input_path.is_file() {
            ...
            for entry in WalkDir::new(input_path)
                .follow_links(false)
                .sort_by(|a, b| a.file_name().cmp(b.file_name())) // deterministic ordering
            {
                let entry = entry?;
                let path = entry.path();
    ...
    fn add_file_to_archive(
        ...
        let mut file = File::open(file_path)
            .with_context(|| format!("failed to open file for archiving: {}", file_path.display()))?;
...
impl CompressionFormat for ZstdFormat {
    fn compress(
        ...
        let encoder = zstd::Encoder::new(output_file, options.level)
            .with_context(|| format!("failed to create zstd encoder with level {}", options.level))?;
        
        // note: threading configuration will be added later
        // the zstd crate may have different parameter APIs
        ...
        for file_path in files_to_add {
            ...
            let archive_path = file_path.strip_prefix(base_path)
    ...
    fn extract(
        ...
        for entry_result in archive.entries()? {
            ...
            if entry_path.components().any(|comp| comp == std::path::Component::ParentDir) {
            ...
            if output_path.exists() && !options.overwrite {
                anyhow::bail!("file already exists: {} (use --overwrite to force)", output_path.display());
            }
