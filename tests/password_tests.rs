...
fn test_zstd_encryption_comprehensive() -> Result<()> {
    ...
    fs::write(input_dir.join("subdir2/nested/binary.bin"), &[0u8, 1u8, 255u8, 127u8])?;
    ...
    assert!(extract_dir.join("complex_input/subdir1/medium.txt").exists());
    assert!(extract_dir.join("complex_input/subdir2/large.txt").exists());
    assert!(extract_dir.join("complex_input/subdir2/nested/binary.bin").exists());
    ...
    assert_eq!(fs::read_to_string(extract_dir.join("complex_input/small.txt"))?, "small file content");
    assert_eq!(fs::read_to_string(extract_dir.join("complex_input/subdir1/medium.txt"))?, "x".repeat(1024));
    assert_eq!(fs::read_to_string(extract_dir.join("complex_input/subdir2/large.txt"))?, "y".repeat(100_000));
    assert_eq!(fs::read(extract_dir.join("complex_input/subdir2/nested/binary.bin"))?, vec![0u8, 1u8, 255u8, 127u8]);
    assert_eq!(fs::read_to_string(extract_dir.join("complex_input/unicode.txt"))?, "Hello 世界 🌍 тест");
    assert_eq!(fs::read_to_string(extract_dir.join("complex_input/empty.txt"))?, "");
...
fn test_zstd_encryption_edge_cases() -> Result<()> {
    ...
    zzz_cmd()
        .args(["compress", "--password", special_password, "-f", "zst", "-o"])
        .arg(&output_archive3)
        .arg(&input_file)
        .assert()
        .success();
...
fn test_zstd_encryption_vs_standard() -> Result<()> {
    ...
    assert!(encrypted_size > standard_size, 
        "Encrypted archive ({} bytes) should be larger than standard archive ({} bytes)", 
...
fn test_zstd_encryption_list_command() -> Result<()> {
    ...
    zzz_cmd()
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("Cannot list encrypted ZSTD archive"));
...
fn test_zstd_encryption_compression_levels() -> Result<()> {
    ...
    for level in [1, 5, 10, 15, 20] {
        let output_archive = tmp_dir.path().join(format!("level_{}.zst", level));
        
        zzz_cmd()
            .args(["compress", "--password", "leveltest", "-f", "zst", "--level", &level.to_string(), "-o"])
            .arg(&output_archive)
            .arg(&input_file)
            .assert()
            .success();
...
fn test_zstd_encryption_corruption_handling() -> Result<()> {
    ...
    assert!(original_data.len() > 50, "Archive should be reasonably sized for corruption testing");
    ...
    for (i, &pos) in corruption_positions.iter().enumerate() {
        ...
        let corrupted_archive = tmp_dir.path().join(format!("corrupted_{}.zst", i));
...
fn test_zstd_encryption_performance_comparison() -> Result<()> {
    ...
    println!("  Standard: {} bytes, {:?}", standard_size, standard_time);
    println!("  Encrypted: {} bytes, {:?}", encrypted_size, encrypted_time);
    ...
    assert!(size_overhead < 3.0, "Encryption overhead too high: {}x", size_overhead);
    ...
    assert!(encrypted_time.as_secs() < 30, "Encryption took too long: {:?}", encrypted_time);
...
fn test_zstd_encryption_format_validation() -> Result<()> {
    ...
    assert!(archive_data.len() > 20, "Archive should have reasonable size");
    
    // Check for our custom magic header "ZSTDECRYPT1"
    let magic_header = b"ZSTDECRYPT1";
    assert_eq!(&archive_data[0..magic_header.len()], magic_header, 
    ...
    assert_ne!(&archive_data[0..4], zstd_magic, 
